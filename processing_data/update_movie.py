...
for i in os.listdir(data_path + 'chunk_ratings'):
    df = pd.read_csv(data_path + 'chunk_ratings/'+i, encoding='ISO-8859-1')
    ...
    for idx, row in df.iterrows():
        ...
        temp_data = (row['numVotes'], row['averageRating'], row['tconst'])        
        try:
            cursor.execute("UPDATE Movies set NumVotes = %s , AverageRating = %s where Movie_id = %s", temp_data)
        except:
            continue
    conn.commit()
