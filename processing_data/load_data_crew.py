...
for i in os.listdir(data_path + 'chunk_name_basic'):
    df = pd.read_csv(data_path + 'chunk_name_basic/'+i, encoding='ISO-8859-1')
    ...
    for idx, row in df.iterrows():
        ...
        temp_data = (row['nconst'], row['primaryName'], row['birthYear'], row['deathYear'], '0')        
        try:
            cursor.execute("INSERT INTO Directors VALUES(%s, %s, %s, %s, %s);", temp_data)
        except:
            continue
    conn.commit()
