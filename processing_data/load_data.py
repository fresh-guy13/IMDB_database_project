...
for i in os.listdir(data_path + 'chunk_title_basic'):
    ...
    for idx, row in df.iterrows():
        ...
        temp_data = (row['tconst'], row['runtimeMinutes'], row['primaryTitle'], row['startYear'], row['endYear'], 0.0, 0.0)
        try:
            cursor.execute("INSERT INTO Movies VALUES(%s, %s, %s, %s, %s, %s, %s);", temp_data)
        except:
            continue
    conn.commit()
